  ...
  async up(queryInterface, Sequelize) {
    ...
    const gestorPermissions = permissoes
      .filter((p) => {
        ...
        if (
          p.perm_nome.startsWith('permissoes_') ||
          p.perm_nome.startsWith('role_permissoes_')
        ) {
          return false;
        }
        return true;
    ...
    const usuarioPermissions = permissoes
      .filter((p) => {
        ...
        if (
          modulosSemAcesso.some((modulo) =>
            p.perm_nome.startsWith(modulo + '_'),
          )
        ) {
          return false;
        }
