...
async function imageToBase64(imagePath, categoria) {
  try {
    ...
    try {
      ...
      const isLogo = nomeArquivo.includes('logo') || nomeArquivo.includes('header') || nomeArquivo.includes('marca');
      const isCarousel = categoria === 'carousel' || nomeArquivo.includes('slide') || nomeArquivo.includes('banner');
      ...
      console.warn(`⚠️ Erro ao converter ${imagePath} para WebP:`, error.message);
      ...
      try {
        ...
        console.warn(`⚠️ Fallback falhou para ${imagePath}:`, fallbackError.message);
        return null;
      }
    }
  } catch (error) {
...
export default {
  async up(queryInterface, Sequelize) {
    ...
    console.log(`✅ Seeder: ${imagens.length} imagens encontradas para inserir`);
