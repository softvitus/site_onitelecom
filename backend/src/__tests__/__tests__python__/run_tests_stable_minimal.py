...
# -*- coding: utf-8 -*-
"""
Test Orchestrator - Testes Estáveis (Sem CREATE/DELETE)
Executa apenas testes de leitura e endpoints que não exploram conta bloqueada
"""
...
class TestOrchestratorStableMinimal:
    """Executa apenas testes estáveis que não criam/deletam"""
    ...
    def _get_admin_token(self):
        """Obter token admin uma única vez"""
        try:
    ...
    def run_all_modules(self):
        """Executar testes estáveis de todos os módulos"""
        
        print("\n" + "="*70)
        print("🚀 TESTES ESTÁVEIS DE INTEGRAÇÃO (Leitura apenas)")
        print("="*70)
        print("Base URL: http://localhost:3000/api/v1")
        print("⚠️  Apenas testes de leitura (sem CREATE/DELETE)")
        print("="*70 + "\n")
        
        if not self.admin_token:
            print("❌ Não foi possível obter token admin")
            return
        
        # Testes de Parceiro (apenas GET)
    ...
    def print_final_report(self):
        ...
        print("\n" + "="*70)
        print("📋 RELATÓRIO FINAL - TESTES ESTÁVEIS")
        print("="*70 + "\n")
        ...
        if self.results['total_failed'] == 0:
            print("✨ TODOS OS TESTES ESTÁVEIS PASSARAM COM SUCESSO!")
        else:
...
def main():
    """Ponto de entrada"""
    orchestrator = TestOrchestratorStableMinimal()
